...
public class CalendarAppointmentController {
    ...
    public ApiResponse<?> saveAppointment(@RequestBody CalendarDto calendarDto) {
        Map<String, Object> resBody = new HashMap<>();
        try {
            resBody = calendarAppointmentService.saveAppointment(calendarDto);
            if(resBody.get("error").equals("")) {
                return new ApiResponse<>(HttpStatus.OK.value(), "Add Appointment Successfully.", resBody);
