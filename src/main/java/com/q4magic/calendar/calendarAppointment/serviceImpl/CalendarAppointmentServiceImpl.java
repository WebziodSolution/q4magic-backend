...
public class CalendarAppointmentServiceImpl implements CalendarAppointmentService {
    ...
    public Map<String, Object> saveAppointment(Map<String, Object> calendarData) {
        ...
        try {
            ...
            reqObj.put("customerName", customerName);
            reqObj.put("eventTitle", eventTitle);
            ...
            String selectNewSlotsUrl = siteUrl + "select-new-slots?v=" + encodedCusId + "&req="
                    + URLEncoder.encode(encodedReq, StandardCharsets.UTF_8.toString());
            ...
            String emailBody = "<!DOCTYPE html>" +
                    ...
                    "<p>To reject or select new time slots for this appointment request, please choose an option below:</p>" +
                    ...
                    + "</td>" +
                    "<td style=\"padding: 0 8px;\">" +
                    "<a href=\"" + selectNewSlotsUrl
                    + "\" target=\"_blank\" style=\"background-color: #44288E; color: #ffffff !important; padding: 10px 22px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px; display: inline-block;\">Select New Slots</a>"
                    + "</td>" +
                    "</tr>" +
                    "</table>" +
                    "</div>" +
    ...
    public Map<String, Object> setAcceptOrRejectAppointment(Map<String, Object> payload) {
        ...
        try {
            ...
            if ("reject".equalsIgnoreCase(status)) {
                ...
                // Check if there is any meeting in calendar for these time slots and delete from calendar, Google & Outlook
                String calTimeZone = reqObj != null && reqObj.has("calTimeZone") ? reqObj.getString("calTimeZone") : "";
                List<Map<String, String>> slotsToCheck = new ArrayList<>();
                if (reqObj != null && reqObj.has("timeSlots")) {
                    JSONArray arr = reqObj.optJSONArray("timeSlots");
                    if (arr != null) {
                        for (int i = 0; i < arr.length(); i++) {
                            JSONObject s = arr.optJSONObject(i);
                            if (s != null) {
                                Map<String, String> m = new HashMap<>();
                                m.put("start", s.optString("start"));
                                m.put("end", s.optString("end"));
                                slotsToCheck.add(m);
                            }
                        }
                    }
                }
                if (payload.get("start") != null && !payload.get("start").toString().trim().isEmpty()) {
                    Map<String, String> m = new HashMap<>();
                    m.put("start", payload.get("start").toString().trim());
                    m.put("end", payload.get("end") != null ? payload.get("end").toString().trim() : "");
                    slotsToCheck.add(m);
                }

                Set<Calendar> meetingsToDelete = new HashSet<>();
                String[] datePatterns = {
                        "MM/dd/yyyy HH:mm:ss", "yyyy-MM-dd HH:mm:ss", "MM/dd/yyyy HH:mm", "yyyy-MM-dd HH:mm"
                };

                for (Map<String, String> slot : slotsToCheck) {
                    String sStart = slot.get("start");
                    if (sStart == null || sStart.trim().isEmpty()) continue;

                    Date parsedDate = null;
                    for (String pattern : datePatterns) {
                        try {
                            SimpleDateFormat sdf = new SimpleDateFormat(pattern, Locale.ENGLISH);
                            parsedDate = sdf.parse(sStart.trim());
                            if (parsedDate != null) break;
                        } catch (Exception ignored) {}
                    }
                    if (parsedDate == null) continue;

                    Date calStartDate = parsedDate;
                    if (calTimeZone != null && !calTimeZone.trim().isEmpty() && !calTimeZone.equalsIgnoreCase(serverDatabaseTimeZone)) {
                        try {
                            SimpleDateFormat dbt = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH);
                            String dbDateStr = this.commonService.convertEventTimeZoneToUserDB(dbt.format(parsedDate), calTimeZone, serverDatabaseTimeZone);
                            calStartDate = dbt.parse(dbDateStr);
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                    }

                    // Direct match by start date/time
                    List<Calendar> matchedByTime = this.calendarRepository.findByCustomerIdAndCalStartDateTime(customers.getId(), calStartDate);
                    if (matchedByTime != null && !matchedByTime.isEmpty()) {
                        meetingsToDelete.addAll(matchedByTime);
                    }

                    // Check day bookings by matching time and date
                    try {
                        SimpleDateFormat dayFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH);
                        String slotDay = dayFormat.format(calStartDate);
                        List<Calendar> dayBookings = this.calendarRepository.findBookList(customers.getId(), slotDay);
                        if (dayBookings != null) {
                            for (Calendar b : dayBookings) {
                                if (b.getCalStartDateTime() != null) {
                                    boolean timeAndDateMatch = Math.abs(b.getCalStartDateTime().getTime() - calStartDate.getTime()) < 5000;
                                    if (timeAndDateMatch) {
                                        meetingsToDelete.add(b);
                                    }
                                }
                            }
                        }
                ...
                // Delete all identified meetings from Google, Outlook, notifications, and local Calendar table
                int deletedCount = 0;
                for (Calendar calToDelete : meetingsToDelete) {
                    if (calToDelete == null || calToDelete.getId() == null) continue;
                    try {
                        List<CalendarDetails> cdList = this.calendarDetailsRepository.findCalendarDetailsList(calToDelete.getId());
                        if (cdList != null) {
                            for (CalendarDetails cd : cdList) {
                                if ("google".equalsIgnoreCase(cd.getCaldType()) && cd.getCaldSycId() != null && !cd.getCaldSycId().isEmpty()) {
                                    if (customers.getGoogleCalendarRefreshToken() != null && !customers.getGoogleCalendarRefreshToken().isEmpty()) {
                                        try {
                                            this.googleCalendarService.deleteEvent(customers.getId(), cd.getCaldSycId());
                                        } catch (Exception ex) {
                                            ex.printStackTrace();
                                        }
                                    }
                                }
                                if ("outlook".equalsIgnoreCase(cd.getCaldType()) && cd.getCaldSycId() != null && !cd.getCaldSycId().isEmpty()) {
                                    if (customers.getOutlookCalendarRefreshToken() != null && !customers.getOutlookCalendarRefreshToken().isEmpty()) {
                                        try {
                                            this.outlookCalendarService.deleteEvent(customers.getId(), cd.getCaldSycId());
                                        } catch (Exception ex) {
                                            ex.printStackTrace();
                                        }
                                    }
                        ...
                        try {
                            List<CalendarNotification> notifications = this.calendarNotificationRepository.findByCalendarId(calToDelete.getId());
                            if (notifications != null && !notifications.isEmpty()) {
                                this.calendarNotificationRepository.deleteAll(notifications);
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                        ...
                        try {
                            this.calendarRepository.deleteByCalId(calToDelete.getId(), customers.getId());
                        } catch (Exception ex) {
                            this.calendarRepository.delete(calToDelete);
                        }
                        deletedCount++;
                    } catch (Exception ex) {
                        ex.printStackTrace();
            ...
            if (payload.get("timeZone") != null && !payload.get("timeZone").toString().trim().isEmpty()) {
                calTimeZone = payload.get("timeZone").toString().trim();
            } else if (payload.get("calTimeZone") != null && !payload.get("calTimeZone").toString().trim().isEmpty()) {
                calTimeZone = payload.get("calTimeZone").toString().trim();
            }
            ...
            if (payload.get("start") != null && !payload.get("start").toString().trim().isEmpty()) {
                selectedStart = payload.get("start").toString().trim();
                selectedEnd = payload.get("end") != null ? payload.get("end").toString().trim() : "";
            } else if (reqObj != null && reqObj.has("timeSlots")) {
            ...
            if (selectedEnd.isEmpty() && !selectedStart.isEmpty()) {
                long durationMinutes = reqObj != null && reqObj.has("slotTimeMinus") ? reqObj.getLong("slotTimeMinus") : 60L;
                try {
                    SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss", Locale.ENGLISH);
                    Date sDate = sdf.parse(selectedStart);
                    Date eDate = new Date(sDate.getTime() + durationMinutes * 60 * 1000);
                    selectedEnd = sdf.format(eDate);
                } catch (Exception ex) {
                    try {
                        SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH);
                        Date sDate = sdf2.parse(selectedStart);
                        Date eDate = new Date(sDate.getTime() + durationMinutes * 60 * 1000);
                        selectedEnd = sdf2.format(eDate);
                    } catch (Exception ignored) {}
                }
            }
