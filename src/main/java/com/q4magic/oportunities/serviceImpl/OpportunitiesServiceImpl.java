...
public class OpportunitiesServiceImpl implements OpportunitiesService {
    ...
    @Override
    public List<OpportunitiesDto> getAllOpportunities(Integer userId, String fetchType, String search, List<String> salesStages, List<String> status) {
        try {
            ...
            if (me.getRole() != null && "SALE MANAGER".equals(me.getRole().getRole())) {
                ...
                if (subs != null) for (Customers s : subs) visibleCustomerIds.add(s.getId());
            }
...
//            }
    ...
    public List<Map<String, Object>> getAllOpportunitiesGroupedByStage(
        ...
        try {
            ...
            for (OpportunitiesDto opp : flatList) {
                if (opp.getId() != null) {
                    List<ClosePlanDto> closePlanDtoList =
                            this.closePlanService.getClosePlanByOppId(opp.getId());
            ...
            for (OpportunitiesDto dto : flatList) {
                ...
                if (oppStatus == null || oppStatus.isBlank()) continue;
            ...
            java.util.function.Function<List<OpportunitiesDto>, Integer> calcTotal = list ->
                    list.stream()
    ...
    public OpportunitiesDto getOpportunityById(Integer id) {
        try {
            Opportunities opportunities = this.opportunitiesRepository.findById(id).orElseThrow(() -> new RuntimeException("Opportunity not found with id: " + id));
            ...
            if (opportunities.getForecastDate() != null) {
                opportunitiesDto.setForecastDate(this.commonService.convertDateToString(opportunities.getForecastDate()));
            }
            ...
            List<OpportunityPartnerDetailsDto> partnerDetails = this.opportunityPartnerDetailsService.getAllOpportunityPartnerDetails(opportunities.getId());
            ...
            if (!closePlanDtoList.isEmpty()) {
                for (ClosePlanDto closePlanDto : closePlanDtoList) {
                    List<ClosePlanNotesDto> closePlanNotesDtoList = this.closePlanNotesService.findLastTwoComments(closePlanDto.getId(), closePlanDto.getContactId());
        ...
        try {
            ...
            if (opportunity.getAccountId() != null) {
                Account account = this.accountRepository.findById(opportunity.getAccountId())
                        .orElseThrow(() -> new RuntimeException("Account not found with id: " + opportunity.getAccountId()));
            ...
            BeanUtils.copyProperties(opportunity, opportunities, "id", "isDeleted", "logo", "dealAmount", "discountPercentage");
            this.opportunitiesRepository.save(opportunities);

            if (opportunity.getNewLogo() != null) {
                String updatedPath = this.commonService.updateFileLocation(opportunity.getNewLogo(), opportunities.getCustomers().getId(), "oppLogo", "oppLogo/" + opportunities.getId());
            ...
            if (opportunities.getStatus().equals("Won")) {
                List<OpportunityProducts> opportunityProductsList = this.opportunityProductsRepository.getByOppId(opportunities.getId());
            ...
            opportunitiesCurrentEnvironmentDto.setVendors("[{\"isChecked\":false,\"value\":\"HubSpot\"},{\"isChecked\":false,\"value\":\"SalesForce\"}]");
            this.opportunitiesCurrentEnvironmentService.addOpportunitiesCurrentEnvironment(opportunitiesCurrentEnvironmentDto);
            ...
            this.opportunitiesCurrentEnvironmentService.addOpportunitiesCurrentEnvironment(opportunitiesCurrentEnvironmentDto2);
            return opportunity;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
    ...
    public OpportunitiesDto updateOpportunity(Integer id, OpportunitiesDto opportunity, Boolean syncToSalesforce) {
        try {
            ...
            BeanUtils.copyProperties(opportunity, opportunities, "id", "isDeleted", "dealAmount", "discountPercentage", "whyDoAnything", "businessValue", "currentEnvironment", "decisionMap");
            this.opportunitiesRepository.save(opportunities);

            if (syncToSalesforce && opportunities.getSalesforceOpportunityId() != null) {
                SyncRecordsQueue syncRecordsQueue = this.syncRecordsQueueRepository.findBySubjectId(id, opportunity.getCreatedBy());
    ...
    public void deleteOpportunity(Integer id, Boolean syncToSalesforce) {
        try {
            ...
            if (syncToSalesforce && opportunities.getSalesforceOpportunityId() != null) {
                ...
                SyncRecordsQueueDto existingSyncRecord = this.syncRecordsQueueService.findBySubjectId(id, opportunities.getCustomers().getId());
    ...
    public String updateOpportunityLogo(Integer userId, Integer oppId, String image) {
        try {
            Opportunities opportunities = this.opportunitiesRepository.findById(oppId).orElseThrow(() -> new RuntimeException("Opportunity not found"));
    ...
    public Map<String, Object> updateOpportunityData(Map<String, Object> data) {
        try {
            ...
            Integer oppId = data.get("opportunityId") != null ? Integer.parseInt(data.get("opportunityId").toString()) : 0;
            ...
            List<Map<String, Object>> currentEnv = data.get("currentEnv") != null ? (List<Map<String, Object>>) data.get("currentEnv") : null;
            ...
            if (oppId != 0 && cusId != 0) {
                ...
                if (currentEnv != null && currentEnvironmentStatus == 0) {
                    List<OpportunitiesCurrentEnvironment> opportunitiesCurrentEnvironmentList = this.opportunitiesCurrentEnvironmentRepository.findByOppId(oppId);
                ...
                if (decisionMap != null) {
                    ...
                    for (Map<String, Object> decision : decisionMap) {
                        String contact_name = decision.get("contact_name") != null ? decision.get("contact_name").toString() : null;
                        String process_name = decision.get("process_name") != null ? decision.get("process_name").toString() : null;
                        String process_date = decision.get("process_date") != null ? decision.get("process_date").toString() : null;
...
//                        }
    ...
    public Map<String, Object> checkOpportunity(Integer oppId) {
        try {
            ...
            res.put("NextSteps", (opportunities.getNextSteps() == null || opportunities.getNextSteps().isEmpty()) ? 0 : 1);
            List<OpportunitiesCurrentEnvironmentDto> opportunitiesCurrentEnvironment = this.opportunitiesCurrentEnvironmentService.getOpportunitiesCurrentEnvironmentByOppId(oppId);
            List<OpportunityContact> opportunityContact = this.opportunityContactRepository.findByOppId(oppId);
            res.put("CurrentEnvironment", (opportunitiesCurrentEnvironment == null || opportunitiesCurrentEnvironment.isEmpty()) ? 0 : 1);
